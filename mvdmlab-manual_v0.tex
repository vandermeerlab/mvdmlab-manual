...
described in more detail in the \href{sec:animalcare}{Animal Care and
  Recordkeeping} section, and (2) lab notes, described in the
\href{sec:notes}{Documentation} section.

\greybox{ \textsf{{\bf Important}: I assume the lab manual and
    procedures on GitHub are accurate. This means that you should
    follow all of the policies and procedures contained in the manual
    and GitHub. If you notice something that seems to be wrong, please
    let me know (for the lab manual) or change it yourself (if on
    GitHub). If there is something in the lab manual or GitHub that
    you notice people aren't doing, please bring this up at lab
    meeting, or to me directly. Don't assume this is okay (it's
    not).}}
...
animals\sidenote{Some good examples are highlighted in the
  \href{https://www.mpg.de/10882259/MPG_Whitepaper.pdf}{``Position
    statement of the Max Planck Society concerning the use of animals
    in experiments for basic research''}}. However, past successes do
not mean we can experiment freely on any animal we want. The use of
animals for research carries a moral, scientific, and legal obligation
to only perform experiments when the benefits can be reasonably
expected to outweigh the costs, to do so only if no suitable
alternatives are available, and to do so while caring for our animals
to the fullest extent possible. These notions are often phrased as the
``3 R's'' (reduction, replacement, refinement)\sidenote{If you will be
  working with animals, you will learn about this in the
  \href{https://about.citiprogram.org/en/homepage/}{CITI training}.}.
...
\item{We strive to {\it design experiments such that the outcomes will
  ...
  mandated by the
  NIH\sidenote{\url{https://grants.nih.gov/policy/reproducibility/index.htm}}. Many
  experimental design decisions can be brought into focus by perparing
  a formal preregistration of your proposed
  experiment\sidenote{\url{https://cos.io/prereg/}}, which requires justifying how
...
{\it animal protocols}\sidenote{{\it Animal protocol}: document
  outlining the scientific rationale, aims, and a set of proposed
  experiments, including specific animal numbers for each. The total
  number of approved animals may not be exceeded.}. Only
procedures\sidenote{{\it Procedure:} Any action or change beyond
  picking an animal up in the vivarium (e.g.\ for changing cages or
  weighing) counts as a procedure. This includes, among others,
  placing an animal on a restricted diet, running a behavioral task,
  and injecting an analgesic.} listed in the animal protocol may be
performed on animals, must be performed as described, and only by
approved personnel listed on the protocol. We are {\bf required} to
log all procedures. Invasive procedures, such as electrode implant
surgery, require follow-up procedures providing post-operative care
(administration of analgesia, monitoring). The lab's currently active
animal protocols can be found in the \#iacuc channel on Slack, and in
a binder in the surgery anteroom (Moore B100).

\greybox{ \textsf{\textbf{Important}: Violations of the above requirements,
    such as performing a procedure not previously approved, or
    neglecting to log a procedure that was performed, are
    ...
    again, and let MvdM know. Honesty is valued.}}
