...
\item{For the data to be interpretable, records need to be kept.}
\item{We take care of our animals. If animals are comfortable and well
  ...
  to handle.}
\item{Data is valuable, and should be protected.}
\item{Data should be shared. To facilitate use by others, it needs to
  be annotated, and stored in a format so that others can use it.}
\end{itemize}
...
\item{If applicable, compress any files that are large and
  ...
  tracking) files\sidenote{See section XXX for a quick primer on data
    compression.}.}
\item{Upload the data to the lab server. See section XXX for
  instructions.}
...
Incoming, InProgress, Promoted

\section{Data use cases}

Typical use cases for data. Your own project. MvdM needs figure for a
grant. New student comes in with a different question. A colleague
wants to check your results.
